...
# Removed: from pycram.robot_descriptions import hsr_description  # Not needed!

def initialize_hsr_pickup():
    """
    Initializes the Toyota HSR in the environment and sets up the necessary
    PyCRAM elements for performing actions.
    """
    # 1. Setup the world and the robot view
    world, robot_view, context, viz = setup_hsrb_in_environment(load_environment=load_environment, with_viz=True)

    # 2. Ensure the HSRB semantic annotation is available
    hsrb = HSRB.from_world(world)

    # 3. Initialize ROS node if needed
    if not rclpy.ok():
        rclpy.init()

    return world, robot_view, context, hsrb


if __name__ == "__main__":
    world, robot_view, context, hsrb = initialize_hsr_pickup()

    print(f"Robot initialized: {hsrb.name}")
    print(f"Arms available: {[arm.name for arm in hsrb.arms]}")
