...
from pycram.external_interfaces import *
import rclpy
...
def take_obj_from_plcmt(location: String, obj: String):
    ...
    finished_object = STLParser(os.path.join(os.path.dirname(__file__), "..", "..", "resources", obj.__str__(),
                                             obj.__str__() + "spoon.stl")).parse()
...
def _location_from_string(location: String):
    match location:
        # This should later contain all possible objects OR just one query possibly
        case "obj":
            obj_room = queries.query_living_room_area(world2)
            obj_pose = PoseStamped().from_list(position=obj_room)
            return obj_pose  # living_room_placeholder
        case _:
