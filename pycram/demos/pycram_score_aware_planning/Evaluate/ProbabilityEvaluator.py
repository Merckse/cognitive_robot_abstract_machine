...
class RobotProbability(Evaluator):
    ...
    def estimate(self, tasks : list[Task]) -> list[ExpectedProbabilityModel]:
        probability = 1
        expected_probability_list = []
        for task in tasks:
            for action in task.task_steps:
                print(action.action_type, action.object_name)
                probability = round(probability * BASE_PROBABILITY.get((action.action_type, action.object_name)), 2)
            expected_probability_list.append(ExpectedProbabilityModel(task_id=task.task_id,expected_probability=probability))
            probability = 1
        sorted_list = sorted(expected_probability_list, key=lambda x: x.expected_probability, reverse=True)
        return sorted_list
    ...
    def summary_estimate(self, estimates: list[ExpectedProbabilityModel]) -> None:
        ...
        for rank, e in enumerate(estimates, 1):
            lines.append(
                f"{rank:<6} {e.task_id:<10} {e.expected_probability:>8} "
            )
        print("\n".join(lines))

if __name__ == "__main__":
    robot_probability = RobotProbability()
    val = robot_probability.estimate(TASKS.get(TaskMode.PP))
